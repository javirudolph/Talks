...
```{r}
# After you've collected block_maxima...

layout(matrix(c(1, 2), nrow = 2), heights = c(1, 2.5))

# ── Top panel: jittered strip ──────────────────────────────────
par(mar = c(0, 4, 3, 1))
plot(block_maxima,
     jitter(rep(0, length(block_maxima)), amount = 0.3),
     pch   = 16,
     cex   = 0.5,
     col   = adjustcolor("gold3", alpha.f = 0.5),
     xaxt  = "n",
     yaxt  = "n",
     xlab  = "",
     ylab  = "",
...
# ── Bottom panel: histogram with GEV curve ─────────────────────
par(mar = c(4, 4, 0, 1))
hist(block_maxima,
     breaks  = 30,
     prob    = TRUE,
     main    = "",
     xlab    = "Maximum value",
     col     = adjustcolor("gold3", alpha.f = 0.55),
     border  = "white")
...
if (requireNamespace("evd", quietly = TRUE)) {
  ...
  curve(evd::dgev(x, loc = loc, scale = scale, shape = shape),
        col = "black", lwd = 2, add = TRUE)
  legend("topright",
         legend = paste0("GEV fit  |  ξ = ", round(shape, 2)),
         col    = "black",
         lwd    = 2,
         bty    = "n")
}
...
## The three domains of attraction {.smaller}

*The same method applied to three different worlds — ξ tells you which world you're in.*
...
set.seed(27)
n         <- 10000
threshold <- 0.90   # use 90th percentile as threshold in each case

# ── Data-generating processes ──────────────────────────────────────────────
frechet_data <- rlnorm(n, meanlog = 0, sdlog = 1.2)   # heavy tail  → ξ > 0
gumbel_data  <- rexp(n, rate = 1)                      # light tail  → ξ ≈ 0
weibull_data <- runif(n, min = 0, max = 1)             # bounded     → ξ < 0

datasets <- list(
  list(data  = frechet_data,
       label = "Lognormal",
       sub   = "Heavy tail — Fréchet domain",
       col   = "#7F77DD",
       xlim  = c(0, 15)),
  list(data  = gumbel_data,
       label = "Exponential",
       sub   = "Light tail — Gumbel domain",
       col   = "#1D9E75",
       xlim  = c(0, 8)),
  list(data  = weibull_data,
       label = "Uniform",
       sub   = "Bounded tail — Weibull domain",
       col   = "#D85A30",
       xlim  = c(0, 1))
)
...
for (d in datasets) {
  ...
  x   <- d$data
  u   <- quantile(x, threshold)
  exc <- x[x > u] - u

  # ── Row 1: population with threshold ────────────────────────────────────
  hist(x,
       breaks  = 50,
       prob    = TRUE,
       main    = d$label,
       xlab    = "Value",
       ylab    = "Density",
       col     = adjustcolor(d$col, alpha.f = 0.55),
       border  = "white",
       xlim    = d$xlim,
       cex.main = 1.1,
       cex.lab  = 0.9,
  ...
  # ── Row 2: GPD fit to exceedances ────────────────────────────────────────
  fit   <- fpot(x, threshold = u)
  sc    <- fit$estimate["scale"]
  sh    <- fit$estimate["shape"]
  label <- paste0("GPD fit  |  ξ = ", round(sh, 2))
  ...
  hist(exc,
       ...
       xlab    = "Excess  (x − u)",
       ylab    = "Density",
       col     = adjustcolor(d$col, alpha.f = 0.35),
       border  = "white",
       cex.main = 1.0,
       cex.lab  = 0.9,
       cex.axis = 0.85)

  curve(dgpd(x, loc = 0, scale = sc, shape = sh),
        col = "grey20",
        lwd = 2.5,
        add = TRUE)
...
mtext(
  expression(
    bold("POT applied to three distributions") ~~ "|" ~~
    "same procedure, same estimator — " *
    xi * " recovers the tail structure each time"
  ),
  outer = TRUE,
  side  = 3,
  cex   = 0.95
...
Animal movement data consistently returns **ξ > 0** — placing dispersal kernels in the Fréchet domain. This is not a modeling assumption. It is an empirical regularity across taxa.
:::

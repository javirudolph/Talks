...
::: {.question-block}
**In disease ecology:** You cannot fully understand an SIR model by only reading its equations.  
You have to **build it, run it, break it, and interpret it** to understand what it means and how to ask/answer questions.
:::
...
## My research: Individual variation as a mechanism {background-color="#ffffff"}

**Across systems, the same question:**

How does variation among individuals drive population-level outcomes?
...
::: {.fragment}
- **Seed dispersal:** Some birds move farther → rare long-distance events shape forest structure
:::

::: {.fragment}
- **Disease transmission:** Some hosts infect more → superspreaders drive outbreak dynamics
:::
...
::: {.fragment}
::: {.key-insight}
**The computational literacy connection:**

My research produces two outputs: scientific contributions and educational infrastructure. The disease models I build become interactive teaching modules, developed with students and centered on metacognition and computational thinking.
:::
:::
...
::: {.notes}
My research centers on individual variation as a mechanistic driver. Across seed dispersal, disease transmission, and metacommunity ecology, the unifying thread is that population-level outcomes are not predictable from species-level averages alone.

This is also where computational literacy becomes essential: these are systems where simulation is not just a convenience, it is the only way to understand what's happening.
:::
...
- Long-distance seed dispersal shapes metapopulation dynamics
- Extreme climatic events drive population crashes
- Rare large fires structure landscapes
- **Superspreading events control outbreak dynamics**

::: {.key-insight}
**The payoff:** EVT lets you predict the probability of events you haven't witnessed yet from the data you have.

This is exactly what we need for wildlife disease systems with sparse data.
:::
...
## The same question, different system {background-color="#0E2841" .white-text}
...
::: {.key-insight}
**The parallel is not metaphorical. It is mechanistic.**
...
If ξ characterizes the dispersal tail, can ξ characterize the transmission tail?
:::
...
- Movement simulator for American mink in Patagonian river networks
- Collaboration with Felipe Hernández (Universidad Austral de Chile) and José Miguel Ponciano (UF)
- Using EVT/POT to parameterize dispersal tails from sparse telemetry
- Connecting to Ponciano-Capistrán stochastic pure birth process for disease spread

**The payoff:**

- Predict pathogen spread potential *before* outbreak data exist
- Identify high-risk river corridors for surveillance
...
::: {.key-insight}
**This is the power of EVT:**

You don't need to observe a 100 km dispersal event to estimate the probability of a 100 km dispersal event.

You fit the tail from the data you have, and the theory tells you what the tail predicts about events you haven't seen.

**This is also the power of computational literacy:**
...
## Another example: Internal structure of metacommunities {background-color="#ffffff"}

:::: {.columns}

::: {.column width="50%"}

**The question:** When do species co-occurrence patterns reflect environmental filtering vs. spatial processes?

**The computational challenge:** Joint Species Distribution Models (jSDMs) are black boxes unless you understand:
- What they assume about species interactions
- What they estimate about environmental responses
- When residual correlations mean something biological vs. statistical artifact

::: {.citation}
Rudolph & Leibold (2020) *Ecology*
:::
...
::: {.key-insight}
**This is a computational literacy problem:**

Students can run `sjSDM()` and get output. But can they:
- Interpret what the latent factors represent?
- Diagnose when the model is overfit?
- Translate between residual correlation matrices and macroecological patterns?

The code runs. The understanding requires moving between the ecological question, the statistical structure, and the computational implementation.
:::

**We teach this:** Advanced ecology course co-taught with Mathew Leibold using National Lakes Assessment data across 9 ecoregions. Students fit sjSDMs and characterize macroecological patterns to ask: when do scaling laws hold, and when do they break?
...
::: {.notes}
The metacommunity work with Mathew Leibold is another case study in computational literacy. Joint Species Distribution Models have become standard tools, but students learn to run them without understanding what they are estimating or when to trust the output.

We co-teach an advanced graduate course where students work with National Lakes Assessment data across 9 ecoregions, fit jSDMs to understand how environmental filtering and spatial processes shape freshwater communities, and connect those process-level results to pattern-level macroecological scaling laws.
:::
...
- Research consulting for quantitative and computational approaches in ecology and biomedicine
...
**The metacommunity course example:**

- Advanced ecology with Mathew Leibold
- National Lakes Assessment data (9,000+ lakes, 9 ecoregions, phytoplankton/zooplankton/benthic macroinvertebrates)
- Each student analyzes an ecoregion pair: macroecological patterns + sjSDMs
- Final symposium: when do scaling laws hold, and what do jSDMs tell us about why?
...
- Movement ecology is the mechanistic link between individual behavior and pathogen spread
...
::: {.key-insight}
**My position exists because computational literacy is now foundational to ecological and biomedical research.**

The question is not whether students will use computation. It is whether they will understand what they are doing when they do.
:::
